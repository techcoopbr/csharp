...
namespace Modelo
{
    class CteCteSeguroDAL
    {
        public void Gravar(CteCteSeguro seg)
        {
            if (seg.Codigo == 0)
                Inserir(seg);
            else
        ...
        private void Inserir(CteCteSeguro seg)
        {

        }

        private void Atualizar(CteCteSeguro seg)
        {

        }

        public void Remover(CteCteSeguro seg)
        {
        ...
        public void PostCteCce()
        {
            ...
            CteCteSeguroDAL sdal = new CteCteSeguroDAL();

            FbDataReader dr = conn.DataReader("select p.*, e.idweb from CTE_CTE_SEGURO p, empresa E where p.sincronizado = 0");

            CteCteSeguro c = new CteCteSeguro();
                ...
                while (dr.Read())
                {
                    ...
                    try
                    {
                        ...
                        using (var streamReaderr = new StreamReader(httpResponsee.GetResponseStream()))
                        {
                            ...
                            conn.ExecuteQueries("UPDATE CTE_CTE_SEGURO P SET P.IDCTECTESEGUROWEB = " + Convert.ToString(DocumentId) + " WHERE P.CODIGO = " + Convert.ToString((int)dr["codigo"]));
                    ...
                    try
                    {
                        ...
                        using (var streamWriter = new StreamWriter(httpWebRequest.GetRequestStream()))
                        {
                            if (Convert.ToInt32(dr["Idweb"]) > 0)
                            {
                                ...
                                if (dr["responsavel"] != DBNull.Value) { c.Responsavel = (int)dr["reponsavel"]; }
                                if (dr["seguradora"] != DBNull.Value) { c.Seguradora = (string)dr["seguradora"]; }
                                if (dr["apolice"] != DBNull.Value) { c.Apolice = (string)dr["apolice"]; }
                                if (dr["averbacao"] != DBNull.Value) { c.Averbacao = (string)dr["averbacao"]; }
                                ...
                                try
                                {
                                    CteCteSeguroDAL segdal = new CteCteSeguroDAL();
                                    //movdal.PostNfproduto((int)dr["codigo"]);

                                    conn.ExecuteQueries("UPDATE CTE_CTE_SEGURO P SET P.SINCRONIZADO = 1 WHERE P.CODIGO = " + Convert.ToString(c.Codigo));
                                }
                                catch
                                {
        ...
        public CteCteSeguro ObterPorId(long id)
        {
            var seg = new CteCteSeguro();
