...
namespace Modelo
{
    class MdfeChavecteDAL
    {
        public void Gravar(MdfeChavecte cha)
        {
            if (cha.Codigo == 0)
                Inserir(cha);
            else
        ...
        private void Inserir(MdfeChavecte cha)
        {

        }

        private void Atualizar(MdfeChavecte cha)
        {

        }

        public void Remover(MdfeChavecte cha)
        {
        ...
        public void PostMdfeChavecte()
        {
            ...
            MdfeChavecteDAL cdal = new MdfeChavecteDAL();

            FbDataReader dr = conn.DataReader("select p.*, e.idweb from MDFE_CHAVECTE p, empresa E where p.sincronizado = 0");

            MdfeChavecte c = new MdfeChavecte();
                ...
                while (dr.Read())
                {
                    ...
                    try
                    {
                        ...
                        using (var streamReaderr = new StreamReader(httpResponsee.GetResponseStream()))
                        {
                            ...
                            conn.ExecuteQueries("UPDATE MDFE_CHAVECTE P SET P.IDMDFECHAVECTEWEB = " + Convert.ToString(DocumentId) + " WHERE P.ID = " + Convert.ToString((int)dr["codigo"]));
                    ...
                    try
                    {
                        ...
                        using (var streamWriter = new StreamWriter(httpWebRequest.GetRequestStream()))
                        {
                            if (Convert.ToInt32(dr["Idweb"]) > 0)
                            {
                                ...
                                if (dr["chavecte"] != DBNull.Value) { c.Chavecte = (string)dr["chavecte"]; }
                                ...
                                try
                                {
                                    MdfeChavecteDAL chadal = new MdfeChavecteDAL();
                                    //movdal.PostNfproduto((int)dr["codigo"]);

                                    conn.ExecuteQueries("UPDATE MDFE_CHAVECTE P SET P.SINCRONIZADO = 1 WHERE P.ID = " + Convert.ToString(c.Codigo));
                                }
                                catch
                                {
        ...
        public MdfeChavecte ObterPorId(long id)
        {
            var cha = new MdfeChavecte();
