...
namespace Modelo
{
    class MdfeCiotDAL
    {
        public void Gravar(MdfeCiot cio)
        {
            if (cio.Codigo == 0)
                Inserir(cio);
            else
        ...
        private void Inserir(MdfeCiot cio)
        {

        }

        private void Atualizar(MdfeCiot cio)
        {

        }

        public void Remover(MdfeCiot cio)
        {

        }


        public void PostMdfeCiot()
        {
            ...
            MdfeCiotDAL cdal = new MdfeCiotDAL();

            FbDataReader dr = conn.DataReader("select p.*, e.idweb from MDFE_CIOT p, empresa E where p.sincronizado = 0");

            MdfeCiot c = new MdfeCiot();
                ...
                while (dr.Read())
                {
                    ...
                    try
                    {
                        ...
                        using (var streamReaderr = new StreamReader(httpResponsee.GetResponseStream()))
                        {
                            ...
                            conn.ExecuteQueries("UPDATE MDFE_CIOT P SET P.IDMDFECIOTWEB = " + Convert.ToString(DocumentId) + " WHERE P.ID = " + Convert.ToString((int)dr["codigo"]));
                    ...
                    try
                    {
                        ...
                        using (var streamWriter = new StreamWriter(httpWebRequest.GetRequestStream()))
                        {
                            if (Convert.ToInt32(dr["Idweb"]) > 0)
                            {
                                ...
                                if (dr["cpfcnpj"] != DBNull.Value) { c.Cpfcnpj = (string)dr["cpfcnpj"]; }
                                ...
                                try
                                {
                                    MdfeCiotDAL ciodal = new MdfeCiotDAL();
                                    //movdal.PostNfproduto((int)dr["codigo"]);

                                    conn.ExecuteQueries("UPDATE MDFE_CIOT P SET P.SINCRONIZADO = 1 WHERE P.ID = " + Convert.ToString(c.Codigo));
                                }
                                catch
                                {
        ...
        public MdfeCiot ObterPorId(long id)
        {
            var cio = new MdfeCiot();
