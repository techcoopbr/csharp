...
namespace Modelo
{
    class MdfeCondutorDAL
    {
        public void Gravar(MdfeCondutor con)
        {
            if (con.Codigo == 0)
                Inserir(con);
            else
        ...
        private void Inserir(MdfeCondutor con)
        {

        }

        private void Atualizar(MdfeCondutor con)
        {

        }

        public void Remover(MdfeCondutor con)
        {

        }


        public void PostMdfeCondutor()
        {
            ...
            MdfeCondutorDAL cdal = new MdfeCondutorDAL();

            FbDataReader dr = conn.DataReader("select p.*, e.idweb from MDFE_CONDUTOR p, empresa E where p.sincronizado = 0");

            MdfeCondutor c = new MdfeCondutor();
                ...
                while (dr.Read())
                {
                    ...
                    try
                    {
                        ...
                        using (var streamReaderr = new StreamReader(httpResponsee.GetResponseStream()))
                        {
                            ...
                            conn.ExecuteQueries("UPDATE MDFE_CONDUTOR P SET P.IDMDFECONDUTORWEB = " + Convert.ToString(DocumentId) + " WHERE P.ID = " + Convert.ToString((int)dr["codigo"]));
                    ...
                    try
                    {
                        ...
                        using (var streamWriter = new StreamWriter(httpWebRequest.GetRequestStream()))
                        {
                            if (Convert.ToInt32(dr["Idweb"]) > 0)
                            {
                                ...
                                if (dr["cpf"] != DBNull.Value) { c.Cpf = (string)dr["cpf"]; }
                                ...
                                try
                                {
                                    MdfeCondutorDAL condal = new MdfeCondutorDAL();
                                    //movdal.PostNfproduto((int)dr["codigo"]);

                                    conn.ExecuteQueries("UPDATE MDFE_CONDUTOR P SET P.SINCRONIZADO = 1 WHERE P.ID = " + Convert.ToString(c.Codigo));
                                }
                                catch
                                {
        ...
        public MdfeCondutor ObterPorId(long id)
        {
            var con = new MdfeCondutor();
