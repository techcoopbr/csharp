...
namespace Modelo
{
    class MdfeDescarregamentoDAL
    {
        public void Gravar(MdfeDescarregamento des)
        {
            if (des.Codigo == 0)
                Inserir(des);
            else
        ...
        private void Inserir(MdfeDescarregamento des)
        {

        }

        private void Atualizar(MdfeDescarregamento des)
        {

        }

        public void Remover(MdfeDescarregamento des)
        {

        }


        public void PostMdfeDescarregamento()
        {
            ...
            MdfeDescarregamentoDAL ddal = new MdfeDescarregamentoDAL();

            FbDataReader dr = conn.DataReader("select p.*, e.idweb from MDFE_DESCARREGAMENTO p, empresa E where p.sincronizado = 0");

            MdfeDescarregamento d = new MdfeDescarregamento();
                ...
                while (dr.Read())
                {
                    ...
                    try
                    {
                        ...
                        using (var streamReaderr = new StreamReader(httpResponsee.GetResponseStream()))
                        {
                            ...
                            conn.ExecuteQueries("UPDATE MDFE_DESCARREGAMENTO P SET P.IDMDFEDESCARREGAMENTOWEB = " + Convert.ToString(DocumentId) + " WHERE P.ID = " + Convert.ToString((int)dr["codigo"]));
                    ...
                    try
                    {
                        ...
                        using (var streamWriter = new StreamWriter(httpWebRequest.GetRequestStream()))
                        {
                            if (Convert.ToInt32(dr["Idweb"]) > 0)
                            {
                                ...
                                d.Idweb = (int)Convert.ToInt32(dr["Idweb"]);

                                if (dr["codigo"] != DBNull.Value) { d.Codigo = (int)dr["codigo"]; }
                                if (dr["idmdfe"] != DBNull.Value) { d.Idmdfe = (int)dr["idmdfe"]; }
                                if (dr["idcidade"] != DBNull.Value) { d.Idcidade = (int)dr["idcidade"]; }
                                if (dr["uf"] != DBNull.Value) { d.Uf = (string)dr["uf"]; }
                                if (dr["ibge"] != DBNull.Value) { d.Ibge = (string)dr["ibge"]; }
                                if (dr["descricao"] != DBNull.Value) { d.Descricao = (string)dr["descricao"]; }
                                if (dr["account_id"] != DBNull.Value) { d.Idweb = (int)dr["account_id"]; }
                                ...
                                try
                                {
                                    MdfeDescarregamentoDAL desdal = new MdfeDescarregamentoDAL();
                                    //movdal.PostNfproduto((int)dr["codigo"]);

                                    conn.ExecuteQueries("UPDATE MDFE_DESCARREGAMENTO P SET P.SINCRONIZADO = 1 WHERE P.ID = " + Convert.ToString(d.Codigo));
                                }
                                catch
                                {
        ...
        public MdfeDescarregamento ObterPorId(long id)
        {
            var des = new MdfeDescarregamento();
