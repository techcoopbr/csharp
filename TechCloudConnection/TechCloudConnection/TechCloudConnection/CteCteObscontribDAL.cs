...
namespace Modelo
{
    class CteCteObscontribDAL
    {
        public void Gravar(CteCteObscontrib obs)
        {
            if (obs.Codigo == 0)
                Inserir(obs);
            else
        ...
        private void Inserir(CteCteObscontrib obs)
        {

        }

        private void Atualizar(CteCteObscontrib obs)
        {

        }

        public void Remover(CteCteObscontrib obs)
        {
        ...
        public void PostCteCce()
        {
            ...
            CteCteObscontribDAL odal = new CteCteObscontribDAL();

            FbDataReader dr = conn.DataReader("select p.*, e.idweb from CTE_CTE_OBSCONTRIB p, empresa E where p.sincronizado = 0");

            CteCteObscontrib c = new CteCteObscontrib();
                ...
                while (dr.Read())
                {
                    ...
                    try
                    {
                        ...
                        using (var streamReaderr = new StreamReader(httpResponsee.GetResponseStream()))
                        {
                            ...
                            conn.ExecuteQueries("UPDATE CTE_CTE_OBSCONTRIB P SET P.IDCTECTEOBSONTRIBWEB = " + Convert.ToString(DocumentId) + " WHERE P.CODIGO = " + Convert.ToString((int)dr["codigo"]));
                    ...
                    try
                    {
                        ...
                        using (var streamWriter = new StreamWriter(httpWebRequest.GetRequestStream()))
                        {
                            if (Convert.ToInt32(dr["Idweb"]) > 0)
                            {
                                ...
                                if (dr["identificador"] != DBNull.Value) { c.Identificador = (int)dr["identificador"]; }
                                if (dr["observacao"] != DBNull.Value) { c.Observacao = (string)dr["observacao"]; }
                                ...
                                try
                                {
                                    ...
                                    using (var streamReaderr = new StreamReader(httpResponsee.GetResponseStream()))
                                    {
                                        ...
                                        conn.ExecuteQueries("UPDATE CTE_CTE_OBSCONTRIB P SET P.IDCTECTEOBSCONTRIBWEB = " + Convert.ToString(DocumentId) + " WHERE P.CODIGO = " + Convert.ToString((int)dr["codigo"]));
                                ...
                                try
                                {
                                    CteCteObscontribDAL obsdal = new CteCteObscontribDAL();
                                    //movdal.PostNfproduto((int)dr["codigo"]);

                                    conn.ExecuteQueries("UPDATE CTE_CTE_OBSCONTRIB P SET P.SINCRONIZADO = 1 WHERE P.CODIGO = " + Convert.ToString(c.Codigo));
                                }
                                catch
                                {
        ...
        public CteCteObscontrib ObterPorId(long id)
        {
            var obs = new CteCteObscontrib();
