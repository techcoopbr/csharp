...
namespace Modelo
{
    class MdfeChavenfeDAL
    {
        public void Gravar(MdfeChavenfe cha)
        {
            if (cha.Codigo == 0)
                Inserir(cha);
            else
        ...
        private void Inserir(MdfeChavenfe cha)
        {

        }

        private void Atualizar(MdfeChavenfe cha)
        {

        }

        public void Remover(MdfeChavenfe cha)
        {

        }


        public void PostMdfeChavenfe()
        {
            ...
            MdfeChavenfeDAL cdal = new MdfeChavenfeDAL();

            FbDataReader dr = conn.DataReader("select p.*, e.idweb from MDFE_CHAVENFE p, empresa E where p.sincronizado = 0");

            MdfeChavenfe c = new MdfeChavenfe();
                ...
                while (dr.Read())
                {
                    ...
                    try
                    {
                        ...
                        using (var streamReaderr = new StreamReader(httpResponsee.GetResponseStream()))
                        {
                            ...
                            conn.ExecuteQueries("UPDATE MDFE_CHAVENFE P SET P.IDMDFECHAVENFEWEB = " + Convert.ToString(DocumentId) + " WHERE P.ID = " + Convert.ToString((int)dr["codigo"]));
                    ...
                    try
                    {
                        ...
                        using (var streamWriter = new StreamWriter(httpWebRequest.GetRequestStream()))
                        {
                            if (Convert.ToInt32(dr["Idweb"]) > 0)
                            {
                                ...
                                if (dr["chavenfe"] != DBNull.Value) { c.Chavenfe = (string)dr["chavenfe"]; }
                                ...
                                try
                                {
                                    MdfeChavenfeDAL chadal = new MdfeChavenfeDAL();
                                    //movdal.PostNfproduto((int)dr["codigo"]);

                                    conn.ExecuteQueries("UPDATE MDFE_CHAVENFE P SET P.SINCRONIZADO = 1 WHERE P.ID = " + Convert.ToString(c.Codigo));
                                }
                                catch
                                {
        ...
        public MdfeChavenfe ObterPorId(long id)
        {
            var cha = new MdfeChavenfe();
